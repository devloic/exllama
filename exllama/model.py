    ...
    def __init__(self, model, batch_size = 1, max_seq_len = -1, copy_from = None):
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if copy_from is None:
                ...
                p_key_states = torch.zeros(self.batch_size, self.config.num_key_value_heads,  int(self.max_seq_len), int(self.config.head_dim), dtype = torch.float16, device = self.model.config.device_map.layers[i])
                p_value_states = torch.zeros(self.batch_size, self.config.num_key_value_heads, int(self.max_seq_len), int(self.config.head_dim), dtype = torch.float16, device = self.model.config.device_map.layers[i])
